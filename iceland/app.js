(function () {
  ...
  function transformData({ overview, accommodation, car, activity, split, lines, flight, expense }) {
    try {
      ...
      const iskRow = oCells.find(r => Array.isArray(r) && r.includes('ISK')) || [];
      const eurRow = oCells.find(r => Array.isArray(r) && r.includes('EUR')) || [];
      ...
      const budgetPerPerson = (oCells && oCells[1]) ? (num(oCells[1][9]) || 100000) : 100000;
      ...
      const d1Row = carCells.find(r => Array.isArray(r) && String(r[0] ?? '').includes('主要駕駛'));
      const d2Row = carCells.find(r => Array.isArray(r) && String(r[0] ?? '').includes('額外駕駛'));
      ...
      console.error("transformData 發生錯誤，將使用預設資料結構:", err);
      return clone(window.STATIC ?? {});
    }
  }
  ...
  window.__syncIcelandBudgetFromSheets = async function () {
    ...
    try {
      ...
      console.error("同步失敗，進入降級程序:", error);
      const cachedData = localStorage.getItem('cached_iceland_budget');
      if (cachedData) {
        window.APP_DATA = JSON.parse(cachedData);
      ...
      window.renderAll?.(); // 確保不論如何都會執行渲染，解開讀取中狀態
    }
  };
})();